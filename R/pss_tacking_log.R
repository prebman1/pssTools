...
pss_tracking_log <- function(cases, week_start_day = 5) {
  ...
  event_complete_df <- tracking_log %>%
    select(record_id, redcap_event_name, event_complete, event_complete_date) %>%
    pivot_wider(names_from = redcap_event_name, values_from = c(event_complete, event_complete_date))
